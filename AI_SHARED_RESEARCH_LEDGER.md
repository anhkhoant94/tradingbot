## 2026-06-11 Codex - H6P cap45 mitigation stress failed; keep H6P research-only

User asked to continue improving model efficiency by distilling prior "best essence" rather than mutating M-score only. Codex followed the active H6P/R46 frontier and tested a narrow concentration/capacity mitigation, not a broad grid.

Smoke artifact: `output/beat_vni30_parallel/r46_h6p_concentration_mitigation_smoke_20260611/`; script: `backtest/r46_h6p_concentration_mitigation_smoke_20260611.py`. Lowering target cap from 55% to 45% produced two first-gate hits: `hv3_cap45` CAGR `51.811333%`, MaxDD `-25.703564%`, recent VNI+30 `6/6`, min edge `32.393719pp`; `hv25_cap45` CAGR `51.220047%`, MaxDD `-25.705831%`, recent `6/6`, min edge `33.693476pp`. Both reduced p95 single-name weight from baseline `57.07%` to about `47.7-47.8%`, but still had high realized concentration and participation.

Stress artifact: `output/beat_vni30_parallel/r46_h6p_mitigation_stress_20260611/`; script: `backtest/r46_h6p_mitigation_stress_20260611.py`. Verdict: **BLOCKED_BY_REMOVE_TOP_KEEP_RESEARCH_ONLY**. `hv3_cap45` fails cost buffer at 18/20bps (`50.591352%` / `49.768056%` CAGR) and fails remove-top1: removing BSR drops recent VNI+30 to `5/6`, min edge `10.772002pp`, CAGR `49.074307%`. `hv25_cap45` also fails cost buffer at 18/20bps (`49.984566%` / `49.165291%`) and fails remove-top1: removing BSR drops recent `5/6`, min edge `9.862459pp`, CAGR `48.247081%`.

Risk snapshot at 15bps: `hv3_cap45` realized max single weight `53.705957%`, p95 `47.689130%`, max participation `414.883068%` ADV20, 169 trades >20% ADV20; `hv25_cap45` realized max `54.921534%`, p95 `47.832738%`, max participation `379.680869%`, 159 trades >20% ADV20. This is better than the original H6P cap55 but still not production-safe.

Decision: cap45 mitigation is useful evidence but not a promotion candidate. Do not migrate H6P/cap45 to dashboard or paper trade. Do not rerun the same "lower cap / lower hv" family as a production fix unless the mechanism changes. Next productive research should target genuinely orthogonal alpha or a two-sleeve ensemble that survives remove-top stress, or an explicit participation-capped simulator with declared AUM limits.
...
## 2026-06-11 Codex - Ez stale local deploy incident and guard

Incident: after fixing copy-trade NAV scaling, Codex deployed `dashboard/` directly from local files while local static JSON/HTML still carried the 2026-06-10 checkpoint. This overwrote the fresher GitHub Actions deployment on `https://ez-trading.vercel.app`; edge live quotes still showed 2026-06-11, but static `dashboard_live_update_status.json`, `full_universe_live_update_status.json`, and `r46_forecast.json` regressed to 2026-06-10.

Resolution: let `dashboard-auto-refresh.yml` rebuild/deploy from fresh sources and verified public Ez now has `dashboard_live_update_status.json` latestPriceDate `2026-06-11`, `full_universe_live_update_status.json` latestPriceDate `2026-06-11` with usableSymbols `688/688`, and `r46_forecast.json` status `COMPUTED`, asOf `2026-06-11`, computedAtICT `2026-06-11 10:02:56`, planDate `2026-06-15`, rows `0`. Public health passed `--require-edge-live --require-vni-history --require-current-forecast --require-execution-desk`; `copy_execution_nav_scaled=true`.

Hardening: `tools/deploy_online_dashboard_from_tokens.py` now has a stale-deploy guard. Before direct Vercel deploy, it compares local `dashboard/r46_forecast.json`, `dashboard/full_universe_live_update_status.json`, and `dashboard/dashboard_live_update_status.json` against the public/edge latest price date and refuses to deploy stale local artifacts unless `ALLOW_STALE_DASHBOARD_DEPLOY=1` is explicitly set for an intentional rollback. Local dashboard artifacts were synced from public after verification.

Follow-up dashboard fix: the `Theo dõi mua` tab is now linked to the same fail-closed current forecast gate used by `Dự kiến giao dịch thứ 2 tới`: `r46_forecast.json` must be `COMPUTED`, `planDate` must equal the current planning Monday, and `asOf` must be at least the live price date. The watchlist summary exposes `forecastLinkedRows`, `forecastLinkedAsOf`, `forecastLinkedPlanDate`, and `forecastLinkedCurrent`; the `Target tuần` flag no longer comes from stale policy planned orders. Public Ez verification after deploy: forecast `COMPUTED`, asOf `2026-06-11`, computedAtICT `2026-06-11 10:10:09`, planDate `2026-06-15`, forecast rows `0`, watchlist rows `14`, forecastLinkedRows `0`, and no watchlist row marked planned.

Second follow-up: user noticed `Theo dõi mua` stayed at 14 names for weeks. Root cause was not the 5-minute/15-minute live lane: `dashboard/data.js` was still based on `output/screening_full_results.csv` asOf `2026-05-29` because `screening-weekly.yml` failed on `2026-06-05` from vnstock guest rate limit during the screening pipeline. Hardening done: `generate_dashboard_data.py` now applies a live price-sensitive overlay from `.cache/backtest/history_clean` before writing `dashboard/data.js`, updating current price, 20D liquidity, technical score, composite score, status, target/stop/upside/R:R, and summary asOf. `screening-weekly.yml` scheduled default workers were reduced from 4 to 1 to avoid vnstock rate-limit failure. `dashboard/_preview/build_v7_real.py` now excludes memo-only static rows from the auto `Theo dõi mua` table; memo-only count is shown as excluded, not treated as auto-scored. Public Ez verification after deploy: `data.js` asOf `2026-06-11`, live overlay baseAsOf `2026-05-29`, updatedSymbols `702`, data candidates `0`, data watch `5`, embedded watch rows `5` (`SHS`, `VIX`, `VHM`, `VNM`, `MSB`), forecast rows `0`, forecast computedAtICT `2026-06-11 10:43:32`, and public health passed.

Third follow-up: performance KPI must be chart-live, not audit-static. Verified public before fix already had CAGR matching the embedded chart through `2026-06-11`, but MaxDD still came from the locked audit value. Fix: `dashboard/_preview/build_v7_real.py` now computes CAGR, MaxDD, Sharpe, model return, VNI return, and chart edge directly from the embedded chart curve after execution-state/live extension; the main KPI card is `Chart live`, not `Audit model`. Build asserts fail if CAGR/MaxDD/chartEndDate differ from the chart. `tools/check_dashboard_public_health.py` added `--require-dynamic-perf`, and both dashboard workflows plus the direct deploy helper now require it. Public Ez verification after deploy: `embedded_perf_cagr=75.63395281530192`, `computed_chart_cagr=75.63395281530192`, `embedded_perf_maxdd=-25.616795200625532`, `computed_chart_maxdd=-25.616795200625532`, `embedded_perf_chart_end_date=2026-06-11`, `dynamic_perf_matches_chart=true`, forecast computedAtICT `2026-06-11 11:48:57`, and public health passed.
## 2026-06-11 Codex - Fix MSB model-ledger P/L cost basis and clarify copy/model tables

User asked why `Lệnh copy đã khớp` showed MSB profit `3.65%` while `Lịch sử giao dịch` showed about `5.8%`. Root cause was a real model-ledger materialization bug, not a valid return-basis difference. In `open_lots_from_trades()`, BUY lots incorrectly preferred `entryPriceK` over actual `executionPriceK`. MSB had a model BUY `39,600` shares at `13.8k` on `2026-05-18` and a BUY thêm `42,200` shares at actual `14.35k` on `2026-05-25`, but that add-on row carried `entryPriceK=13.8`; the materialized sell therefore treated all `81,800` shares as if cost basis were `13.8k`, creating the wrong `5.778317%` return.

Fix in `dashboard/_preview/build_v7_real.py`: BUY lots now use `executionPriceK`, then `priceK`, then `price`, with `entryPriceK` only as fallback. The same patch adds `Giá bán` and `Giá vốn` columns to `Lệnh copy đã khớp`, `Lệnh model gần nhất`, and `Lịch sử giao dịch`. Copy table meta says `P/L theo giá vốn tài khoản copy`; model/latest and ledger say `P/L theo giá vốn model, không phải tài khoản copy`.

Verification: generator push triggered `dashboard-auto-refresh.yml` run `27329770903`, completed SUCCESS. Public Ez health passed `--require-edge-live --require-vni-history --require-current-forecast --require-execution-desk --require-dynamic-perf`. Public embedded data now has copy MSB SELL `3,800` shares, sell `14.7k`, entry `14.084k`, P/L `1.9568024` million, return `3.645326%`; model/latest and ledger MSB SELL now use weighted entry `14.083608978k`, P/L `0.0421659380` billion, return `3.648204%`, source `execution_state_model_materialized`. The former model `5.778317%` is fixed.
