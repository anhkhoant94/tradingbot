...
## 2026-06-11 Codex - Clarify MSB copy P/L vs model-ledger P/L basis

User asked why `Lệnh copy đã khớp` showed MSB profit `3.65%` while `Lịch sử giao dịch` showed about `5.8%`. Public Ez source confirmed this is not the same return basis. `r46_execution_state.json` / embedded `executionState.orders` has the actual copy-account sell: MSB SELL `2026-06-08`, `3,800` shares, sell `14.7k`, copy entry/cost basis `14.084k`, P/L `1.9568024` million, return `3.645326%`, hold `14` days. Embedded `tradesLatest` / `ledger` has the model/rebased ledger sell: MSB SELL `2026-06-08`, `81,800` shares after model NAV scale, sell `14.7k`, model entry basis `13.8k`, P/L `0.0654408826` billion, return `5.778317%`, source `execution_state_model_materialized`.

Fix: `dashboard/_preview/build_v7_real.py` now shows `Giá bán` and `Giá vốn` in `Lệnh copy đã khớp`, `Lệnh model gần nhất`, and `Lịch sử giao dịch`. Copy table meta says `P/L theo giá vốn tài khoản copy`; model/latest and ledger say `P/L theo giá vốn model, không phải tài khoản copy`. This prevents comparing copy-account return to model-ledger return as if they were the same account.

Verification: pushed only the generator to GitHub, let `dashboard-auto-refresh.yml` run `27329076934`, completed SUCCESS. Public Ez health passed `--require-edge-live --require-vni-history --require-current-forecast --require-execution-desk --require-dynamic-perf`. Public HTML now contains the new labels/headers, and embedded public data still has copy MSB `entryPriceK=14.084`, `returnPct=3.645326%`, while model/latest and ledger MSB have `entryPriceK=13.8`, `returnPct=5.778317%`.
