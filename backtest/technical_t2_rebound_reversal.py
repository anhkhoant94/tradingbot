...
OUT = STATE_DIR / "rebound_reversal"
...
PROFILES = {
    "recovery_focus": {
        ...
        "narrow_leadership": {"exposure": 0.65, "max_holdings": 5, "max_weight": 0.22},
        "recovery": {"exposure": 0.90, "max_holdings": 5, "max_weight": 0.25},
        "risk_off": {"exposure": 0.0, "max_holdings": 0, "max_weight": 0.0},
    },
    "balanced_rebound": {
        "broad_trend": {"exposure": 0.85, "max_holdings": 6, "max_weight": 0.22},
        "narrow_leadership": {"exposure": 0.70, "max_holdings": 5, "max_weight": 0.25},
        "recovery": {"exposure": 0.75, "max_holdings": 5, "max_weight": 0.25},
        "risk_off": {"exposure": 0.0, "max_holdings": 0, "max_weight": 0.0},
    },
}
...
def rebound_score(week: pd.DataFrame, mode: str) -> pd.Series:
    z = v1.robust_z
    prior_drawdown = -week["ret_26w"].clip(upper=0)
    if mode == "rebound":
        return (
            0.25 * z(week["ret_4w"])
            + 0.20 * z(week["ret_8w"])
            + 0.20 * z(prior_drawdown)
            + 0.15 * z(week["rs13_mom_4w"])
            + 0.10 * z(week["volume_expansion_20_60"])
            + 0.10 * z(week["sma20_gap"])
        )
    if mode == "reclaim":
        return (
            0.25 * z(week["sma50_gap"])
            + 0.20 * z(week["ret_4w"])
            + 0.20 * z(week["rs13_mom_4w"])
            + 0.15 * z(week["downside_resilience"])
            + 0.10 * z(week["vol_contraction"])
            + 0.10 * z(week["avg_value_20d_bil"])
        )
    if mode == "rebound_composite":
        return 0.55 * rebound_score(week, "rebound") + 0.45 * v2.subblend_score(week, "recovery", "cash")
    raise ValueError(mode)


def build_targets(profile: str, schedule: str, mode: str, entry_band: float):
    states = v4.load_states()
    states["recent_risk_8w"] = states["raw_state"].eq("risk_off").rolling(8, min_periods=1).sum()
    panel = load_panel()
    panel = panel[
    ...
    for idx, st in enumerate(states.itertuples(index=False)):
        ...
        if bool(getattr(st, "recovery_guard", False)) and state == "recovery":
            params["exposure"] = min(float(params["exposure"]), 0.45)
        ...
        if week.empty:
            continue
        use_rebound = state == "recovery" or int(getattr(st, "recent_risk_8w", 0)) >= 2 or float(getattr(st, "vni_ret_13w", 0)) < 0.02
        if use_rebound:
            week["score"] = rebound_score(week, mode)
            week = week[
                (week["ret_4w"] > -0.03)
                & (week["sma20_gap"] > -0.04)
                & (week["rs13_mom_4w"] > -0.05)
                & (week["avg_value_20d_bil"] >= 5.0)
            ].copy()
        else:
            week["score"] = v2.subblend_score(week, state, "cash")
...
def run_grid() -> None:
    ...
    rows = []
    for profile in ["recovery_focus", "balanced_rebound"]:
        for schedule in ["weekly", "biweekly_state"]:
            for mode in ["rebound", "reclaim", "rebound_composite"]:
    ...
    lines = [
        "# Technical T2 Rebound Reversal",
        "",
        f"Verdict: **{status['verdict']}**",
        "",
        "Pure technical post-bear rebound/reclaim family for recovery regimes.",
        "",
        "## Best / Stress Result",
        "",
    ...
    v1.atomic_write_text(OUT / "rebound_reversal_verdict.md", "\n".join(lines))
