...
model_summary_cards = [
    ...
    ["Tín hiệu", "Tín hiệu tuần được cập nhật trước phiên thực hiện, kèm giá mục tiêu và ngưỡng rủi ro."],
    ["Thực hiện", "Lệnh quy đổi theo NAV copy, làm tròn lot 100, theo dõi target/stop và trạng thái có thể bán."],
    ["Kiểm soát", "Theo dõi trạng thái thị trường, tỷ trọng vị thế và lịch sử lệnh để copy đúng kỷ luật."],
]
...
<div class="content">
  <section class="view on" data-view="copy">
    ...
    <section class="sec">
      <div class="sech"><h2>Lệnh copy đã khớp <span class="scaletag" id="copyExecScaleTag">Theo NAV nhập</span></h2><span class="meta">{len(copy_executions)} lệnh · P/L theo giá vốn tài khoản copy</span></div>
      <table><thead><tr><th>Ngày</th><th>Mã</th><th>Lệnh</th><th class="num">KL</th><th class="num">Giá bán</th><th class="num">Giá vốn</th><th class="num">Giá trị</th><th class="num">Tỷ trọng NAV</th><th class="num">P/L</th><th class="num">P/L %</th><th>Trạng thái</th></tr></thead><tbody id="copyExecRows"></tbody></table>
    </section>
    <section class="sec">
    ...
    <section class="sec"><div class="sech"><h2>Lệnh model gần nhất <span class="scaletag">Model NAV 2021 = 1 tỷ</span></h2><span class="meta">P/L theo giá vốn model, không phải tài khoản copy</span></div><table><thead><tr><th>Ngày</th><th>Mã</th><th>Lệnh</th><th class="num">KL</th><th class="num">Giá bán</th><th class="num">Giá vốn</th><th class="num">Tỷ trọng NAV</th><th class="num">P/L</th><th class="num">P/L %</th><th>Lý do</th></tr></thead><tbody id="latestRows"></tbody></table></section>
  </section>
  ...
  <section class="view" data-view="ledger"><div class="pageh"><div><h1>Lịch sử giao dịch</h1><div class="sub">Full bộ lệnh model đã khớp · P/L theo giá vốn model · quy đổi theo {ledger_basis_label}</div></div></div><section class="sec"><div class="sech"><h2>Lịch sử model <span class="scaletag">Model NAV 2021 = 1 tỷ</span></h2><span class="meta">{len(ledger_rows)} dòng từ {ledger_first_trade_date} đến {ledger_last_trade_date}</span></div><div class="ledgerbar"><b id="ledgerCount"></b><input id="ledgerSearch" placeholder="Tìm theo mã, lệnh, lý do..." /><div class="pager" id="pager"></div></div><div class="scroll"><table><thead><tr><th>Ngày</th><th>Mã</th><th>Lệnh</th><th class="num">KL</th><th class="num">Giá bán</th><th class="num">Giá vốn</th><th class="num">Giá trị</th><th class="num">Tỷ trọng NAV</th><th class="num">P/L</th><th class="num">P/L %</th><th class="num">Nắm giữ</th><th>Lý do</th></tr></thead><tbody id="ledgerBody"></tbody></table></div></section></section>
</div>
</main>
</div>
<script>
...
let globalQ='';
function searchNorm(s){{ return vnPlain(String(s??'')).replace(/\\s+/g,' ').trim(); }}
function searchBlob(v){{ if(v===null||v===undefined) return ''; if(Array.isArray(v)) return v.map(searchBlob).join(' '); if(typeof v==='object') return Object.values(v).map(searchBlob).join(' '); return String(v); }}
function rowMatches(row){{ const q=searchNorm(globalQ); return !q || searchNorm(searchBlob(row)).includes(q); }}
function emptyRow(cols,msg='Không có dòng khớp tìm kiếm.'){{ return `<tr><td colspan="${{cols}}">${{esc(msg)}}</td></tr>`; }}
function liveBadgeLabel(latest, updated){{ const up=String(updated||'').trim(); const day=String(latest||'').trim(); if(!up) return day||'-'; return up.startsWith(day)?up:(day?day+' '+up:up); }}
...
function copyExecutionBaseNavMil(r){{ return Number(r.baseNavMil || D.executionState?.baseNavMil || D.copyAccount?.navMil || 1000) || 1000; }}
function scaledCopyExecution(r, navBil){{ const baseNavMil=copyExecutionBaseNavMil(r); const scale=baseNavMil>0?(navBil*1000)/baseNavMil:navBil; const shares=roundLot(Number(r.shares||0)*scale); const price=Number(r.executionPriceK||0); const grossFromShares=shares&&price?shares*price/1000:null; const grossMil=grossFromShares!==null?grossFromShares:(r.grossMil==null?null:Number(r.grossMil)*scale); const baseGrossMil=Number(r.grossMil||0); const feeRate=baseGrossMil>0&&r.feesMil!=null?Number(r.feesMil)/baseGrossMil:null; const feesMil=grossMil!==null&&feeRate!==null?grossMil*feeRate:(r.feesMil==null?null:Number(r.feesMil)*scale); const entry=Number(r.entryPriceK||0); let pnlMil=r.pnlMil==null?null:Number(r.pnlMil)*scale; if(pnlMil!==null&&entry&&price&&shares) pnlMil=(price-entry)*shares/1000-(feesMil||0); const tradeWeightPct=navBil>0&&grossMil!==null?grossMil/(navBil*1000)*100:null; return {{...r, shares, grossMil, feesMil, pnlMil, tradeWeightPct}}; }}
function copyExecutionHtml(rows, cols=11, navBil=parseNavValue(document.getElementById('navInput')?.value)||1){{ return rows.length ? rows.map(raw=>{{ const r=scaledCopyExecution(raw,navBil); return `<tr><td>${{esc(r.date||'-')}}</td><td><strong>${{esc(r.symbol)}}</strong></td><td>${{pill(r.actionLabel||r.side)}}</td><td class="num">${{r.shares?f(r.shares,0):'-'}}</td><td class="num">${{priceK(r.executionPriceK)}}</td><td class="num">${{priceK(r.entryPriceK)}}</td><td class="num">${{r.grossMil==null?'-':valueTr(r.grossMil)}}</td><td class="num">${{r.tradeWeightPct==null?'-':wp(r.tradeWeightPct,2)}}</td><td class="num ${{cls(r.pnlMil)}}">${{r.pnlMil==null?'-':money(r.pnlMil)}}</td><td class="num ${{cls(r.returnPct)}}">${{pc(r.returnPct,2)}}</td><td>${{pill(r.status||'-')}}</td></tr>`; }}).join('') : emptyRow(cols, globalQ?'Không có lệnh copy khớp tìm kiếm.':'Chưa có lệnh copy đã khớp.'); }}
function renderCopyExecutions(navBil=parseNavValue(document.getElementById('navInput')?.value)||1){{ const rows=copyExecutions.filter(rowMatches); const tag=document.getElementById('copyExecScaleTag'); if(tag) tag.textContent='NAV '+navLabel(navBil)+' tỷ'; const copyBody=document.getElementById('copyExecRows'); if(copyBody) copyBody.innerHTML=copyExecutionHtml(rows,11,navBil); }}
...
async function refreshEdgeLiveStatus(){{ const syms=liveSymbols(); try{{ const res=await fetch('/api/live-status?symbols='+encodeURIComponent(syms.join(',')), {{ cache:'no-store' }}); if(!res.ok) throw new Error('HTTP '+res.status); applyEdgeLiveStatus(await res.json()); }}catch(err){{ const liveStatus=document.getElementById('liveStatusText'); if(liveStatus&&!liveStatus.textContent.includes('edge lỗi')) liveStatus.innerHTML += ' <span class="meta">edge lỗi</span>'; }} }}
...
function renderWatchRows(){{ const rows=D.watchlist.filter(rowMatches); document.getElementById('watchRows').innerHTML = rows.length ? rows.map(w=>`<tr><td><strong>${{esc(w.symbol)}}</strong><div class="meta">${{esc(w.industry||'-')}}</div></td><td>${{w.bucket==='BUY_SOON'?'<span class="pill buy">Có thể mua</span>':'<span class="pill hold">Theo dõi</span>'}}</td><td class="num">${{w.gatePassCount}}/${{w.gateTotal}}</td><td class="num ${{Number(w.upsidePct)>=12?'pos':''}}">${{pc(w.upsidePct)}}</td><td class="num pos">${{priceK(w.targetPrice)}}</td><td class="num ${{Number(w.riskReward)>=2?'pos':''}}">${{w.riskReward==null?'-':f(w.riskReward,2)+'x'}}</td><td class="num">${{w.liq20dBil==null?'-':f(w.liq20dBil,1)}}</td><td>${{w.planned?'Có':'Không'}}</td><td>${{w.buySignal?'Có':'Không'}}</td><td>${{esc(w.note)}}</td></tr>`).join('') : emptyRow(10, globalQ?'Không có mã khớp tìm kiếm.':'Chưa có mã phù hợp cho watchlist.'); }}
function renderLogicGrid(){{ const rows=D.modelSummaryCards.filter(rowMatches); document.getElementById('logicGrid').innerHTML = rows.length ? rows.map((c,i)=>`<article class="logicCard"><h3>${{esc(c[0]||('Card '+(i+1)))}}</h3><p>${{esc(c[1]||'')}}</p></article>`).join('') : '<article class="logicCard"><h3>Không có kết quả</h3><p>Không có nội dung model khớp tìm kiếm.</p></article>'; }}
renderWatchRows();
renderLogicGrid();
